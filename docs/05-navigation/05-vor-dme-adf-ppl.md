# Радионавигация: будущий слой [Part-FCL](../reference/glossary.md#term-part-fcl) {#vor-dme-adf-ppl}

## Назначение {#purpose}

Эта глава явно отделена как **[PART-FCL — ОБЩЕЕ][part-fcl]**. Она вводит [всенаправленный VHF-радиомаяк (English: VHF omnidirectional range, VOR; español: radiofaro omnidireccional VHF)][vor], [дальномерное оборудование (English: distance measuring equipment, DME; español: equipo medidor de distancia)][dme], [автоматический радиокомпас (English: automatic direction finder, ADF; español: radiogoniómetro automático)][adf] и [ненаправленный радиомаяк (English: non-directional beacon, NDB; español: radiofaro no direccional)][ndb]. Текущая GU09 для [ULM](../reference/glossary.md#term-ulm) на pp. 28–32 включает принципы GNSS/оборудования, но не перечисляет подробную глубину этих наземных средств. Глава добавлена для будущего перехода к [LAPL(A)](../reference/glossary.md#term-lapl-a)/[PPL(A)](../reference/glossary.md#term-ppl-a), не как право применять радиосредство без подготовки и исправного оборудования.
...
1. отличить радиал [VOR][vor] от станции (FROM) от пеленга на станцию (TO);
...
|---|---|
| [ULM — ОСНОВА][ulm] | GU09 прямо включает GNSS, но не перечисляет подробную теорию наземных средств [VOR][vor]/DME/ADF; это граница программы, а не эксплуатационный «приоритет» одного средства. |
...
| [PPL — РАСШИРЕНИЕ] | Теоретическая программа радионавигации общая с LAPL; обе ветви проходят её в своей утверждённой подготовке. |
...
**Дано:** положение на радиале [VOR][vor] `060°` от станции.
...
- Радиал [VOR][vor] направлен FROM station.
...
### Q-NAV-021 — Где находится воздушное судно на радиале [VOR][vor] 060? {#q-nav-021}
...
**Почему:** Радиал [VOR][vor] 060 определяется магнитным направлением от станции; он не задаёт расстояние или курс носа.

**Почему главный отвлекающий вариант неверен:** B обращает радиал к станции и путает его с обратным пеленгом TO.

### Q-NAV-022 — Что нужно сделать, прежде чем доверять показанию [VOR][vor]? {#q-nav-022}
...
C. Сверить только буквенный идентификатор, не проверяя флаг и текущий статус станции.<br>
D. Принять устойчивое отклонение стрелки за достаточную проверку геометрии без сопоставления с положением.

**Правильный ответ:** A.

**Почему:** Правильное применение [VOR][vor] начинается с достоверной идентификации станции, проверки статуса показания и геометрической перекрёстной проверки.
...
B. Далеко от станции при малой высоте, потому что горизонтальная составляющая становится наибольшей.<br>
C. На любой высоте одинаково, потому что DME всегда показывает горизонтальное расстояние.<br>
D. Высоко, но только далеко от станции, где вертикальная составляющая якобы доминирует.
...
B. Для ADF/NDB учитывают только девиацию компаса; ночь, гроза, берег и рельеф не влияют на радиопеленг.<br>
C. Ночной эффект относится только к [VOR][vor], поэтому после захода Солнца пеленг NDB становится точнее.<br>
D. Устойчивое показание стрелки доказывает правильную идентификацию и отсутствие помех.

**Правильный ответ:** A.

**Почему:** Пеленг ADF/NDB подвержен влияниям распространения и приёма, включая ночь, статику, берег и рельеф.

**Почему главный отвлекающий вариант неверен:** B сводит ошибку ADF/NDB к девиации компаса и игнорирует перечисленные влияния распространения и приёма.

### Q-NAV-025 — Откуда брать текущие сведения [VOR][vor]/DME в Испании? {#q-nav-025}

A. Из сохранённого перечня станций без даты, если идентификатор знаком.<br>
B. Из текущих ENR 4.1/AD 2 и [NOTAM](../reference/glossary.md#term-notam) для времени операции.<br>
C. Из сохранённой учебной карточки частот, если идентификатор станции совпадает.<br>
D. Только из общего раздела [AIP](../reference/glossary.md#term-aip) GEN, не проверяя ENR 4.1/AD 2 и [NOTAM](../reference/glossary.md#term-notam) для времени операции.
...
- `SRC-AESA-ULM-LEARNING-OBJECTIVES-GU09-ED01` — Navegación, pp. 28–32: GPS/equipment principles, не detailed [VOR][vor]/DME/ADF; проверено 13.07.2026.
...
[ndb]: ../reference/glossary.md#term-non-directional-beacon-ndb
