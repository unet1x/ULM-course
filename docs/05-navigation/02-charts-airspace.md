...
3. построить цепочку [AIP](../reference/glossary.md#term-aip) → SUP/[AIC](../reference/glossary.md#term-aic) → [NOTAM](../reference/glossary.md#term-notam) → AIRAC;
...
| [PPL — РАСШИРЕНИЕ] | Добавляются более широкие задачи радионавигации и планирования. |
...
| [ПРОВЕРИТЬ ПЕРЕД ПОЛЁТОМ] | Редакция, дата вступления в силу (English: with effect from, WEF), исправления, SUP/[AIC](../reference/glossary.md#term-aic)/[NOTAM](../reference/glossary.md#term-notam) и AIRAC. |

## Теория {#theory}

### Масштаб, проекция и измерение {#chart-scale}

Масштаб `1:500 000` означает: одна единица на карте равна 500 000 тех же единиц на земле. Удобный вывод: `1 cm = 5 km`. Измеряют вдоль намеченной линии, а не по случайной дороге; затем переводят единицы и округляют только в конце. **Равноугольная коническая проекция Ламберта (English: Lambert conformal conic projection; español: proyección cónica conforme de Lambert)** сохраняет локальные углы, но не делает любую прямую универсально точной дугой большого круга. Назначение и проекция испанской VFR500 опубликованы в GEN 3.2 (`SRC-ENAIRE-AIP-NAVIGATION-2026`, проверено 13.07.2026).
...
Легенда карты важнее «похожести значка». GEN 2.3 задаёт условные обозначения, GEN 3.2 — серии карт. **Абсолютная отметка (English: elevation; español: elevación)** относится к точке или поверхности над средним уровнем моря (English: mean sea level, MSL; español: nivel medio del mar). Высота препятствия может иметь несколько подписей, смысл которых проверяется по легенде. Контур рельефа не гарантирует безопасный запас: учитываются высота полёта, неопределённость, ветер, облачность, характеристики самолёта и путь отхода. Один заметный ориентир подтверждают формой рельефа, направлением и временем.
...
`VFR500_Changes` датирован 28.05.2026 и перечисляет существенные изменения после печати; [ENAIRE](../reference/glossary.md#term-enaire) указывает InsigniaVFR как постоянно обновляемое веб-представление. Практический порядок: выбрать нужный лист → прочитать редакцию и дату данных → применить `VFR500_Changes` → проверить текущие [AIP](../reference/glossary.md#term-aip)/SUP/[AIC](../reference/glossary.md#term-aic)/[NOTAM](../reference/glossary.md#term-notam) и AIRAC → сверить InsigniaVFR → записать дату источника в лог. Источники: `SRC-ENAIRE-VFR500-2026`; `SRC-ENAIRE-AIP-NAVIGATION-2026`, GEN 3.2 §§4.2, 5, 8 (проверено 13.07.2026).

### [AIP](../reference/glossary.md#term-aip), SUP, [AIC](../reference/glossary.md#term-aic), [NOTAM](../reference/glossary.md#term-notam) и AIRAC {#aip-sup-aic-notam-airac}
...
- принять опубликованный AIRAC за уже вступивший в силу;
...
### Q-NAV-007 — Почему AIRAC 07/26 нельзя было использовать как действующий 13.07.2026? {#q-nav-007}

A. Любой AIRAC действует только после отдельного разрешения пилота.<br>
B. AIRAC 07/26 был опубликован, но дата вступления в силу наступала 06.08.2026.<br>
C. AIRAC применяется только к IFR и никогда к картам [VFR](../reference/glossary.md#term-vfr).<br>
D. AIRAC отменяется каждым [NOTAM](../reference/glossary.md#term-notam) в день публикации.

**Правильный ответ:** B.

**Почему:** Опубликованный AIRAC 07/26 оставался будущим до установленной даты 06.08.2026, поэтому 13.07.2026 применялась версия VIGOR.

**Почему главный отвлекающий вариант неверен:** C неверно исключает [VFR](../reference/glossary.md#term-vfr): AIRAC может менять существенную для эксплуатации аэронавигационную информацию маршрута.
...
D. Что будущий AIRAC уже вступил в силу.
...
**Почему:** Печатная VFR500 обновляется с учётом статуса конкретного листа, VFR500_Changes и текущих [AIP](../reference/glossary.md#term-aip)/SUP/[AIC](../reference/glossary.md#term-aic)/[NOTAM](../reference/glossary.md#term-notam)/AIRAC; InsigniaVFR даёт дополнительную перекрёстную проверку.
...
- `SRC-ENAIRE-AIP-ESPANA` — snapshot 09.07.2026, AIRAC 06/26 и AMDT 408/26; проверено 13.07.2026.
